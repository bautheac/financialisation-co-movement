...
download.file("https://www.dropbox.com/scl/fi/n7t8g3hn32d8e5gf2rcti/references.bib?rlkey=xy251n44q3y0t3omxpqk0dvyf&dl=1", path)
...
table <- get_results(tables, "stats - whole") %>%
...
table <- get_results(tables, "stats - regimes") %>% 
  kbl(format = output_type, booktabs = T, longtable = T, caption = caption, col.names = colnames, align = align) %>%
  column_spec(1L, bold = TRUE) %>%
...
table <- get_results(tables, "regressions - US returns ~ US CHP") %>% 
  kbl(format = output_type, booktabs = T, longtable = T, caption = caption, col.names = colnames, align = align) %>%
  column_spec(1L, bold = TRUE) %>%
...
caption <- if (latex) caption_latex(x = caption) else if (html) caption
colnames <- c("country", "sector", "subsector", "decade", 0L:9L)
align <- make_series_of_repeated_chars(list(l = 4L, r = 10L))

table <- get_results(tables, "correlations - years") %>% 
  kbl(format = output_type, booktabs = T, longtable = T, caption = caption, col.names = colnames, align = align) %>%
  column_spec(1L, bold = TRUE) %>%
  collapse_rows(columns = 1L:4L, valign = tables_collapse_rows_align, longtable_clean_cut = tables_cut, latex_hline = tables_hline) %>%
  style_table(output_type)
