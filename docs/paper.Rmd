...
knitr::asis_output('\\savegeometry{tables}')
...
```{r, include=latex}
knitr::asis_output('\\newpage')
knitr::asis_output('\\loadgeometry{tables}')
```
...
table <- get_results(tables, "descriptive statistics (no regimes)") %>%
  kbl(format = output_type, booktabs = T, longtable = T, caption = caption, col.names = colnames, align = align) %>%
...
table <- get_results(tables, "descriptive statistics (regimes)") %>% 
  kbl(format = output_type, booktabs = T, longtable = T, caption = caption, col.names = colnames, align = align) %>%
  column_spec(1L, bold = TRUE) %>%
...
table <- get_results(tables, "regressions - time series - US commodities ~ CHP") %>% 
  kbl(format = output_type, booktabs = T, longtable = T, caption = caption, col.names = colnames, align = align) %>%
  column_spec(1L, bold = TRUE) %>%
...
table <- get_results(tables, "regressions - time series - US commodities ~ factors from US commodities") %>%
  dplyr::mutate(
    factor = as.character(factor),
    factor = ifelse(factor == "OI nearby", "open interest", factor)
  ) %>% 
  kbl(format = output_type, longtable = T, booktabs = T, caption = caption, col.names = colnames, align = align) %>%
  column_spec(1L, bold = TRUE) %>%
  collapse_rows(columns = 1L:2L, valign = tables_collapse_rows_align, longtable_clean_cut = tables_cut, latex_hline = tables_hline) 
...
table <- get_results(tables, "proportions") %>%
  dplyr::mutate(period = stringr::str_replace_all(period, "present", "post-crisis")) %>%
  dplyr::mutate(
    factor = as.character(factor),
    factor = ifelse(factor == "OI nearby", "open interest", factor)
  ) %>% 
  kbl(format = output_type, longtable = T, booktabs = T, caption = caption, align = align, col.names = colnames) %>%
  column_spec(1L, bold = TRUE) %>%
  collapse_rows(
...
table <- get_results(tables, "average correlations (amongst factor picks)") %>%
...
```{r co-movement-time-reg-picks-facts-picks, fig.pos = 'htbp'}
...
table <- get_results(tables, "regressions - time series - factor picks ~ factors from picks") %>% 
  kbl(format = output_type, longtable = T, booktabs = T, caption = caption, align = align, col.names = colnames) %>%
  column_spec(1L, bold = TRUE) %>%
  collapse_rows(
...
table <- get_results(tables, "regressions - time series - US commodities ~ factors from picks") %>% 
  kbl(format = output_type, longtable = T, booktabs = T, caption = caption, align = align, col.names = colnames) %>%
  column_spec(1L, bold = TRUE) %>%
  collapse_rows(
    columns = 1L:3L, valign = tables_collapse_rows_align, longtable_clean_cut = tables_cut, latex_hline = tables_hline
...
table <- get_results(tables, "regressions - time series - UK metals ~ factors from picks") %>% 
  kbl(format = output_type, longtable = T, booktabs = T, caption = caption, align = align, col.names = colnames) %>%
  column_spec(1L, bold = TRUE) %>%
  collapse_rows(
    columns = 1L:3L, valign = tables_collapse_rows_align, longtable_clean_cut = tables_cut, latex_hline = tables_hline
