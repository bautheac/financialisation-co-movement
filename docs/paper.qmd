...
Over the second period the pattern essentially reverses. For the twenty-four US commodities over this period, fourteen have higher mean returns during phases of **contango** over **backwardation**. Four of the six GB metals achieved higher mean returns over **contango** relative to **backwardation** phases with the difference being statistically significant at the 5% level for one commodity (Nickel-primary (XLME)). The equally weighted US portfolio had a mean return of 15.7% over **backwardation** against 16.2% during phases of **contango**, both statistically significant at the 10% level. The difference was considerably more pronounced for the equally weighted GB metals portfolio with a mean return of 8% during **backwardation** rising to 35.6% during **contango**.   
...
#| tbl-cap: "This table shows mean returns and volatilities for the individual commodities and two equally weighted portfolios of US and GB traded assets for the four periods. For each period, estimates are computed independently over the whole period--figure shown--as well as over phases of **backwardation** (b) and **contango** (c)--results in brackets. The table further shows the results of t-tests of significance for the mean returns computed over the whole periods and t-tests of difference between these when computed over the two phases independently along with F-tests of difference for the corresponding variances. Values significant at the 1%, 5% and 10% level are marked with \\\\*\\\\*\\\\*, \\\\*\\\\* and \\\\* respectively. Detailed results are available in the attached [Internet Appendix](https://bautheac.shinyapps.io/co-movement/). **Backwardation** and **contango** regimes as well as period definitions are discussed in \\@sec-methods while the results are discussed in \\@sec-results."

table_name <- "stats-individuals - regime difference tests"
columns_to_collapse_rows_for <- c("asset")
colnames <- c("asset", "estimate", "past", "financialisation", "crisis", "post-crisis")
align <- make_series_of_repeated_chars(list(l = 1L, r = 5L))
longtable <- TRUE
bold_columns <- 1L:6L 
coloured_columns <- 3L:6L
...
```{r}
#| label: tbl-correlations-inner-years
#| tbl-cap: "While \\@tbl-correlations-inner-periods shows results computed independently for the four periods, this table shows the results of the same analysis implemented for each year over the entire sample period. The results are discussed in \\@sec-results while \\@sec-taxonomy details the commodity assets taxonomy."


table_name <- "correlations - inner - years"
columns_to_collapse_rows_for <- c("country", "sector", "subsector")
colnames <- c(sectors, "decade", 0L:9L)
align <- make_series_of_repeated_chars(list(l = 4L, r = 10L))
longtable <- TRUE
bold_columns <- 1L:14L
coloured_columns <- 5L:14L
