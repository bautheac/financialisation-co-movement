...
# stats ########################################################################
## descriptive #################################################################
descriptive <- readr::read_rds(file.path(app_dir, "results", "descriptive-statistics.rds")) |>
  lapply(function(x) list(`Individual assets` = x))


## regime tests ################################################################
regime_tests_raw <- readr::read_rds(file.path(app_dir, "results", "regime-difference-tests.rds")) 
...
correlations_inner_raw <- readr::read_rds(file.path(app_dir, "results", "correlations-inner.rds")) 
...
correlations_cross_split <- purrr::map(c("US", "global"), function(x){
  ...
  results <- readr::read_rds(file.path(app_dir, "results", file_name))
...
regressions_index_raw <- readr::read_rds(file.path(app_dir, "results", "regressions-index.rds"))
...
regressions_factors_raw <- readr::read_rds(file.path(app_dir, "results", "regressions-factors.rds"))
