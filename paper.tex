...
(XLME)---show a lower figure with five of them significant---Crude
...
volatilities for the individual commodities as well as for two equally
weighted portfolios of US and GB traded assets across the four periods.
For each period, estimates are computed independently over the whole
period (figure shown) as well as over phases of \textbf{backwardation}
(b) and \textbf{contango} (c) with corresponding comparison results
reported in brackets. The table further shows the results of t-tests of
significance for the mean returns computed over the whole periods as
well as t-tests of difference between these when computed over the two
phases independently along with F-tests of difference for the
corresponding variances. Values significant at the 1\%, 5\% and 10\%
level are marked with ***, ** and * respectively. The estimate values
per regime as well as the p-values for the corresponding difference
tests are available in the attached
\href{http://bautheac.duckdns.org:3838/bautheac/co-movement/}{Internet
